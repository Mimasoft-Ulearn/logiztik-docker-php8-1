...
class Other_records_model extends Crud_model {
	...
	function get_values_of_record_fixed_form($id_form) {
		
		$valores_formularios_fijos_table = $this->db->dbprefix('valores_formularios_fijos');
				
		$sql = "SELECT $valores_formularios_fijos_table.*";
		$sql .= " FROM $valores_formularios_fijos_table";
		$sql .= " WHERE $valores_formularios_fijos_table.deleted = 0";
		$sql .= " AND $valores_formularios_fijos_table.id_formulario = $id_form";
