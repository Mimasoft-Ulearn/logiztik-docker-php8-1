...
class Crud_bd_fc_model extends CI_Model {
    ...
    function get_one_where($where = array()) {
        $result = $this->bd_mimasoft_fc->get_where($this->table, $where, 1);
        if ($result->num_rows()) {
            return $result->row();
        } else {
            $db_fields = $this->bd_mimasoft_fc->list_fields($this->table);
            $fields = new stdClass();
    ...
    function get_all_where($where = array(), $limit = 1000000, $offset = 0) {
        ...
        if ($where_in) {
            foreach ($where_in as $key => $value) {
                $this->bd_mimasoft_fc->where_in($key, $value);
            }
            unset($where["where_in"]);
        }
        return $this->bd_mimasoft_fc->get_where($this->table, $where, $limit, $offset);
    }
    ...
    function save(&$data = array(), $id = 0) {
        if ($id) {
            ...
            if ($success) {
                if ($this->log_activity) {
                    ...
                    if (count($fields_changed)) {
                        ...
                        $activity_log_id = $this->bd_mimasoft_fc->insert_id();
            ...
            //insert
            if ($this->bd_mimasoft_fc->insert($this->table, $data)) {
                $insert_id = $this->bd_mimasoft_fc->insert_id();
                if ($this->log_activity) {
                    //log this activity
                    $log_for_id = 0;
                    if ($this->log_for_key) {
    ...
    function update_where($data = array(), $where = array()) {
        if (count($where)) {
            if ($this->bd_mimasoft_fc->update($this->table, $data, $where)) {
    ...
    function delete($id = 0, $undo = false) {
        ...
        $this->bd_mimasoft_fc->where("id", $id);
        $success = $this->bd_mimasoft_fc->update($this->table, $data);
        if ($success) {
            if ($this->log_activity) {
                if ($undo) {
    ...
    protected function prepare_custom_field_query_string($related_to, $custom_fields, $related_to_table) {
        ...
        $join_string = "";
        $select_string = "";
        $custom_field_values_table = $this->bd_mimasoft_fc->dbprefix('custom_field_values');
