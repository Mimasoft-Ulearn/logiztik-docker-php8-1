...
class Projects_model extends Crud_model {
    ...
    */
    ...
    function get_details($options = array()){
        ...
        $sql = "SELECT $projects_table.*, $clients_table.company_name, $industries_table.nombre as rubro, $technologies_table.nombre as subrubro";
        $sql .= " FROM $projects_table";
        $sql .= " LEFT JOIN $clients_table ON $clients_table.id = $projects_table.client_id";
        $sql .= " LEFT JOIN $industries_table ON $industries_table.id = $projects_table.id_industria";
        $sql .= " LEFT JOIN $technologies_table ON $technologies_table.id = $projects_table.id_tecnologia";
        $sql .= " WHERE $projects_table.deleted = 0";
        $sql .= " $where";
        $sql .= " ORDER BY $projects_table.id ASC";
