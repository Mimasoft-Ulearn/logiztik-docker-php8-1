	...
	function get_functional_units(){
		...
		if($client_info->habilitado){
			...
			if($unidad_funcional->id){
				echo $this->load->view("functional_units_water/client/functional_unit_by_date", $view_data, TRUE);
			} else {
				echo $this->load->view("functional_units_water/client/functional_units_by_date", $view_data, TRUE);
			}
		}else{
			$this->session->sess_destroy();
			redirect('signin/index/disabled');
