...
class Unit_processes_carbon extends MY_Controller {
    ...
    function index() {
		...
		if ($this->login_user->user_type === "staff") {
			...
			$id_proyecto = $this->session->project_context;
			//$footprints = $this->Footprints_model->get_footprints_of_methodology(2)->result(); // Metodología con id 2: GHG Protocol
			//$footprint_ids = array();
			...
			if($client_info->habilitado){
				$this->template->rander("unit_processes_carbon/client/index", $view_data);
			}else{
    ...
    private function _make_row($data, $uf_data, $start_date = NULL, $end_date = NULL) {
		...
		$footprints = $this->Footprints_model->get_footprints_of_methodology(3)->result(); // Metodología con id 2: GHG Protocol
		$footprint_ids = array();
		foreach($footprints as $footprint){
			$footprint_ids[] = $footprint->id;
		}
	...
	function get_pdf(){
        ...
        $this->pdf->SetTitle($client_info->sigla."_".$project_info->sigla."_".lang("unit_processes_carbon_pdf")."_".date('Y-m-d'));
        $this->pdf->SetSubject($client_info->sigla."_".$project_info->sigla."_".lang("unit_processes_carbon_pdf")."_".date('Y-m-d'));
		...
		if($id_unidad_funcional){ // SI SE FILTRA POR UNIDAD FUNCIONAL
			$html = $this->load->view('unit_processes_carbon/client/pdf_view_by_uf', $view_data, true);
		} else {
			$html = $this->load->view('unit_processes_carbon/client/pdf_view', $view_data, true);
		}
		...
		$pdf_file_name = $client_info->sigla."_".$project_info->sigla."_".lang("unit_processes_carbon_pdf")."_".date('Y-m-d').".pdf";
	...
	function get_pdf_bak(){
		...
		// var_dump($array_unidades_funcionales);exit;
		// create new PDF document
        $this->load->library('Pdf');
	...
	function get_unit_processes(){
		...
		*/
		...
		$options_footprint_ids = array("footprint_ids" => $footprint_ids);
		// Huellas de la metodología GHG asociadas al Proyecto obtenidas en formato JSON
		$view_data["columnas"] = $this->Project_rel_footprints_model->get_footprints_of_project_json($id_proyecto, $options_footprint_ids);
		//$huellas = $this->Project_rel_footprints_model->get_footprints_of_project($id_proyecto, $options_footprint_ids)->result();
		...
		if($client_info->habilitado){
			if($unidad_funcional->id){
				echo $this->load->view("unit_processes_carbon/client/unit_process_by_date", $view_data, TRUE);
			} else {
				echo $this->load->view("unit_processes_carbon/client/unit_processes_by_date", $view_data, TRUE);
			}
		}else{
			$this->session->sess_destroy();
			redirect('signin/index/disabled');
