...
class Client_waste_detail extends MY_Controller {
	...
	function index(){
		...
		$array_categorias[] = array("id" => "", "text" => "- ".lang("waste_names")." -");
		...
		$sources_dropdown[] = array("id" => "", "text" => "- ".lang("source")." -");
		$sources = $this->Sources_model->get_all()->result();
		foreach($sources as $source){
			$sources_dropdown[] = array("id" => $source->id, "text" => lang($source->name));
		}

		$view_data['sources_dropdown'] = json_encode($sources_dropdown);
	...
	function list_data_table($id_project,$id_material,$date_since,$date_until){
		...
		$options = array(
			...
			"id_source" => $this->input->post("id_source"), // FILTRO APPTABLE
			"id_categoria" => $this->input->post("id_categoria"), // FILTRO APPTABLE
			"group_by" => array("id_source", "id_categoria")
		);

		$forms_data = $this->Form_values_model->get_forms_values_group_by_category_and_source($id_project,"Residuo", $options)->result();
		//var_dump($forms_data);
		$list_data = array();
			...
			foreach($forms_data as $form_data){
				...
				if($categorie->id == $data["id_categoria"]){
					if(isset($data["fecha_retiro"])){
						...
						if($verificacion_fecha_registro == true){
							if($verificacion_fecha == true){
								$data["total_unidad_residuo"] = $form_data->total_unidad_residuo;
								$list_data[$form_data->id] = $data;
							}
						}
					}
	...
	function _make_row($data, $id_project, $id_form_value){
		...
		// FUENTE
		$source = $this->Sources_model->get_one($data["id_source"]);
		$source_name = lang($source->name);
		...
		// TIPO DE RESIDUO
		$material = $this->Materials_model->get_material_of_category($data["id_categoria"])->row();

		// CÓDIGO BASILEA
		$sinader = $this->Sinader_code_model->get_one_where(array(
			"id_client" => $this->login_user->client_id,
			"id_category" => $data["id_categoria"]
		));
		$codigo_basilea = $sinader->ler_code ? $sinader->ler_code : "-";

		// CANTIDAD POR MES
		$unidad_value = to_number_project_format($data["total_unidad_residuo"], $id_project)." (".$data["unidad"].")";
		
		$row_data = array(
			$source_name, // FUENTE
			$nombre_categoria, // NOMBRE RESIDUO
			$material->nombre, // TIPO DE RESIDUO
			$codigo_basilea, // CÓDIGO BASILEA
			$unidad_value, // CANTIDAD POR MES
		);
	...
	*/
	...
	function get_merged_pdf(){
			...
			foreach($forms_data as $form_data){
				...
				if($categorie->id == $data["id_categoria"]){
					if(isset($data["fecha_retiro"])){
						...
						if($verificacion_fecha_registro == true){
							if($verificacion_fecha == true){
								$data["id_formulario"] = $form_data->id_formulario;
								$list_data[$form_data->id] = $data;
							}
						}
					}
		...
		$pdf_merger = new Pdf_merger;

		$array_siglas_materials = array(
			"29" => "RSD",
			"30" => "RNP",
			"31" => "RIL",
			"32" => "RLD",
		...
		if(isset($array_siglas_materials[$id_material])){
			$sigla_material = $array_siglas_materials[$id_material];
		} else {
			$sigla_material = str_replace(" ", "_", $material->nombre);
		}

		$pdf_merge_filename = $info_proyecto->sigla."_".$sigla_material."_".date("mY").".pdf";

		$file_cont = 0;
		foreach($list_data as $id_elemento => $data){

			$id_formulario = $data["id_formulario"];

			$filename_archivo_retiro = $data["nombre_archivo_retiro"];
			$file_url = "files/mimasoft_files/client_".$id_cliente."/project_".$id_proyecto."/form_".$id_formulario."/elemento_".$id_elemento."/".$filename_archivo_retiro;

			if($_SERVER['CI_ENVIRONMENT'] == 'production'){
				$original_file_url = getcwd().'/'.$file_url;
				$parsed_file_url = getcwd().'/'.$file_url.'.parsed.pdf';
				exec('gs -sDEVICE=pdfwrite -o "'.$parsed_file_url.'" -dCompatibilityLevel="1.4" -dNOPAUSE -dQUIET -dBATCH "'.$original_file_url.'"');

				if($filename_archivo_retiro && file_exists($parsed_file_url)){

					// Si el nombre de archivo contiene una letra "ñ",
					if (strpos($filename_archivo_retiro, 'ñ') !== false) {
						$filename_archivo_retiro_n = str_replace("ñ", "n", $filename_archivo_retiro);
						$file_url_n_retiro = "files/temp/".$filename_archivo_retiro_n;
						copy($parsed_file_url, $file_url_n_retiro);
						$pdf_merger->addPDF($file_url_n_retiro, 'all');
					} else {
						$pdf_merger->addPDF($parsed_file_url, 'all');
					}
					$file_cont++;

				}
			}else{
				if($filename_archivo_retiro && file_exists($file_url)){
					...
					if (strpos($filename_archivo_retiro, 'ñ') !== false) {
						$filename_archivo_retiro_n = str_replace("ñ", "n", $filename_archivo_retiro);
						$file_url_n_retiro = "files/temp/".$filename_archivo_retiro_n;
						copy($file_url, $file_url_n_retiro);
						$pdf_merger->addPDF($file_url_n_retiro, 'all');
					} else {
						$pdf_merger->addPDF($file_url, 'all');
					}
					*/
					$pdf_merger->addPDF($file_url, 'all');
					$file_cont++;
			...
			$filename_archivo_recepcion = $data["nombre_archivo_recepcion"];
			$file_url = "files/mimasoft_files/client_".$id_cliente."/project_".$id_proyecto."/form_".$id_formulario."/elemento_".$id_elemento."/".$filename_archivo_recepcion;
			...
			if($_SERVER['CI_ENVIRONMENT'] == 'production'){
				...
				if($filename_archivo_recepcion && file_exists($parsed_file_url)){

					// Si el nombre de archivo contiene una letra "ñ",
					if (strpos($filename_archivo_recepcion, 'ñ') !== false) {
						$filename_archivo_recepcion_n = str_replace("ñ", "n", $filename_archivo_recepcion);
						$file_url_n_recepcion = "files/temp/".$filename_archivo_recepcion_n;
						copy($parsed_file_url, $file_url_n_recepcion);
						$pdf_merger->addPDF($file_url_n_recepcion, 'all');
					} else {
						$pdf_merger->addPDF($parsed_file_url, 'all');
					}
					$file_cont++;
				...
				if($filename_archivo_recepcion && file_exists($file_url)){
					$pdf_merger->addPDF($file_url, 'all');
					$file_cont++;
				}
			}

			$filename_archivo_waste_manifest = $data["nombre_archivo_waste_manifest"];
			$file_url = "files/mimasoft_files/client_".$id_cliente."/project_".$id_proyecto."/form_".$id_formulario."/elemento_".$id_elemento."/".$filename_archivo_waste_manifest;
			...
			if($_SERVER['CI_ENVIRONMENT'] == 'production'){
				...
				if($filename_archivo_waste_manifest && file_exists($parsed_file_url)){

					// Si el nombre de archivo contiene una letra "ñ",
					if (strpos($filename_archivo_waste_manifest, 'ñ') !== false) {
						$filename_archivo_waste_manifest_n = str_replace("ñ", "n", $filename_archivo_waste_manifest);
						$file_url_n_waste_manifest = "files/temp/".$filename_archivo_waste_manifest_n;
						copy($parsed_file_url, $file_url_n_waste_manifest);
						$pdf_merger->addPDF($file_url_n_waste_manifest, 'all');
					} else {
						$pdf_merger->addPDF($parsed_file_url, 'all');
					}
					$file_cont++;
				...
				if($filename_archivo_waste_manifest && file_exists($file_url)){
					$pdf_merger->addPDF($file_url, 'all');
					$file_cont++;
				}
			}
		...
		$pdf_merge_file_url = "files/temp/merged_pdf_elements/";
		if (!is_dir($pdf_merge_file_url)) {
            if (!mkdir($pdf_merge_file_url, 0777, true)) {
		...
		if($file_cont > 0){
			...
			$pdf_merger->merge('file', $pdf_merge_file_url.$pdf_merge_filename);

			// Eliminar temporales de archivos cambiados de "ñ" a "n".
			if(file_exists($file_url_n_retiro)) {
				unlink($file_url_n_retiro);
			}
			if(file_exists($file_url_n_recepcion)) {
				unlink($file_url_n_recepcion);
			}
			if(file_exists($file_url_n_waste_manifest)) {
				unlink($file_url_n_waste_manifest);
			}

			$respuesta = array(
				"success" => true,
				"file_url" => $pdf_merge_file_url.$pdf_merge_filename,
				"filename" => $pdf_merge_filename
			);
		} else {
			$respuesta = array("success" => false, "message" => "No hay archivos disponibles para exportar");
		}
	...
	/* DECLARACIÓN SINADER */
	function list_data_declaration_sinader($id_project, $id_material, $date_since, $date_until){
		/**
		...
		$options = array(
			...
			"id_tratamiento" => $this->input->post("id_tratamiento"),
			"group_by" => array("id_tratamiento", "id_source", "id_categoria")
		);

		$list_data = array();

		$categories = $this->Categories_model->get_category_of_material($id_material)->result();
		//$forms_data = $this->Form_values_model->get_forms_values_of_forms_by_flux($id_project, "Residuo", $options)->result();
		$forms_data = $this->Form_values_model->get_forms_values_group_by_category_and_source($id_project,"Residuo", $options)->result();
	...
	function _make_row_declaration_sinader($data, $id_project, $key){
		...
		// TRATAMIENTO
		$tratamiento = $this->Tipo_tratamiento_model->get_one($data["tipo_tratamiento"]);

		// NOMBRE EMPRESA RECEPTORA | RUC EMPRESA | CÓDIGO DE REGISTRO DE EMPRESA 
		$empresa_receptora_residuos = $this->Fixed_feeder_waste_receiving_companies_values_model->get_one($data["id_waste_receiving_company"]);
		...
		$row_data = array(
			...
			$codigo_basilea,
			$tratamiento->nombre,
			$empresa_receptora_residuos->company_name,
			$empresa_receptora_residuos->company_rut,
			$empresa_receptora_residuos->company_code,
			$unidad_value,
		);
		...
		$row_data = array(
			$nombre_categoria,
			$generator_establishment_rut,
			$generator_establishment_code,
			$treatment,
			$quantity,
			$carrier_rut,
			$patent,
	...
	function get_excel_declaration_sinader(){
		...
		$nombre_columnas[] = array("nombre_columna" => lang("generator_establishment_rut_m"), "id_tipo_campo" => "generator_establishment_rut");
		$nombre_columnas[] = array("nombre_columna" => lang("generator_establishment_code_m"), "id_tipo_campo" => "generator_establishment_code");
		$nombre_columnas[] = array("nombre_columna" => lang("code_receiving_treatment_m"), "id_tipo_campo" => "treatment");
		...
		foreach($result as $res){
			...
			foreach($nombre_columnas as $index_columnas => $columna){
				...
				if(!is_array($columna)){
					...
					if($columna["id_tipo_campo"] == "ler" || $columna["id_tipo_campo"] == "generator_establishment_rut"
					|| $columna["id_tipo_campo"] == "generator_establishment_code" || $columna["id_tipo_campo"] == "treatment"
