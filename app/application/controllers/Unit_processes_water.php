...
class Unit_processes_water extends MY_Controller {
    ...
    function index() {
		...
		if ($this->login_user->user_type === "staff") {
			...
			$this->template->rander("unit_processes_carbon/index", $view_data);
			...
			//$huellas_fijas = ',{"title":"'.lang("water_in_product").'<br /> (m3 agua EQ)", "class": "text-right dt-head-center"}';
			//$huellas_fijas .= ',{"title":"'.lang("consumed_water").'<br /> (m3 agua EQ)", "class": "text-right dt-head-center"}';
			//$columnas = $this->Project_rel_footprints_model->get_footprints_of_project_json($id_proyecto, $options_footprint_ids);
			//$view_data["columnas"] = $columnas.$huellas_fijas;
			...
			if($client_info->habilitado){
				$this->template->rander("unit_processes_water/client/index", $view_data);
			}else{
    ...
    function modal_form() {
        ...
        $this->load->view('unit_processes_carbon/modal_form', $view_data);
    }
	...
	private function _admin_make_row($data){
		...
		$row_data = array($data->id,
            modal_anchor(get_uri("unit_processes_carbon/admin_view/" . $data->id), $data->nombre, array("title" => lang('view_unit_process'))),
        ...
        $row_data[] = modal_anchor(get_uri("unit_processes_carbon/admin_view/" . $data->id), "<i class='fa fa-eye'></i>", array("title" => lang('view_unit_process')))
					. modal_anchor(get_uri("unit_processes_carbon/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_unit_process'), "data-post-id" => $data->id))
                	. js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_unit_process'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("unit_processes_carbon/delete"), "data-action" => "delete-confirmation"));
	...
	function admin_view($id = 0) {
        ...
        if ($id) {
            ...
            if ($model_info) { 
                ...
                $this->load->view('unit_processes_carbon/view', $view_data);
    ...
    private function _make_row($data, $uf_data, $start_date = NULL, $end_date = NULL) {
		...
		$array_categorias_mayores_huellas_fijas = array();
		...
		foreach($huellas as $huella){
			...
			if($huella->id == 30){
				$total_huella_ud = $total_pu;
			}
			if($huella->id == 31){
				$total_huella_ui = $total_pu;
			}
			if($huella->id == 32){
				$total_huella_sl = $total_pu;
			}
			if($huella->id == 33){
				$total_huella_se = $total_pu;
			}
			//
		...
		// HUELLA FIJA 
		$total_huella_ap = ($total_huella_ud + $total_huella_ui) - ($total_huella_sl + $total_huella_se);
		$total_huella_ap_final = to_number_project_format($total_huella_ap, $id_proyecto);
		$row_data[] = $total_huella_ap_final;

		$total_huella_ac = $total_huella_ap + $total_huella_se;
		$total_huella_ac_final = to_number_project_format($total_huella_ac, $id_proyecto);
		$row_data[] = $total_huella_ac_final;
		...
		// agregar a $array_categorias_formatted los datos de categorías para las huellas fijas (2)
		$array_nombres_huellas_dinámicas = array(
			30 => $this->Footprints_model->get_one(30)->nombre,
			31 => $this->Footprints_model->get_one(31)->nombre,
			32 => $this->Footprints_model->get_one(32)->nombre,
			33 => $this->Footprints_model->get_one(33)->nombre,
		);
		...
		foreach($array_categorias as $nombre_categoria => $array_huellas){
			...
			$valor_ud = $array_huellas[$array_nombres_huellas_dinámicas[30]];
			$valor_ui = $array_huellas[$array_nombres_huellas_dinámicas[31]];
			$valor_sl = $array_huellas[$array_nombres_huellas_dinámicas[32]];
			$valor_se = $array_huellas[$array_nombres_huellas_dinámicas[33]];

			$total_valor_ap = ($valor_ud + $valor_ui) - ($valor_sl + $valor_se);
			$total_valor_ap_final = to_number_project_format($total_valor_ap, $id_proyecto);
			$total_valor_ac = $total_valor_ap + $valor_se;
			$total_valor_ac_final = to_number_project_format($total_valor_ac, $id_proyecto);

			$array_categorias_formatted[$nombre_categoria][lang("water_in_product")] = $total_valor_ap_final;
			$array_categorias_formatted[$nombre_categoria][lang("consumed_water")] = $total_valor_ac_final;

			$array_categorias_mayores_huellas_fijas[lang("water_in_product")][$nombre_categoria] = $total_valor_ap;
			$array_categorias_mayores_huellas_fijas[lang("consumed_water")][$nombre_categoria] = $total_valor_ac;
		...
		$max_categoria_water_in_product = array_keys($array_categorias_mayores_huellas_fijas[lang("water_in_product")], max($array_categorias_mayores_huellas_fijas[lang("water_in_product")]));
		$max_categoria_consumed_water = array_keys($array_categorias_mayores_huellas_fijas[lang("consumed_water")], max($array_categorias_mayores_huellas_fijas[lang("consumed_water")]));
		$array_categorias_mayores[lang("water_in_product")] = $max_categoria_water_in_product;
		$array_categorias_mayores[lang("consumed_water")] = $max_categoria_consumed_water;

		$row_data['num_huellas'] = count($huellas) + 2; // + 2 huellas fijas
	...
	// Versión que NO recalcula los valores para las huellas, en cambio los recibe por Post desde el lado cliente
	function get_pdf(){
		...
		// Se obtienen los datos de la tabla para usarlos en el pdf, de esta manera no es necesario volver a calcular los datos
		$appTable_data = json_decode($this->input->post('appTable_data'), true);
		$view_data['appTable_data'] = $appTable_data;
        ...
        $this->pdf->SetTitle($client_info->sigla."_".$project_info->sigla."_".lang("unit_processes_water_pdf")."_".date('Y-m-d'));
        $this->pdf->SetSubject($client_info->sigla."_".$project_info->sigla."_".lang("unit_processes_water_pdf")."_".date('Y-m-d'));
		...
		if($id_unidad_funcional){ // SI SE FILTRA POR UNIDAD FUNCIONAL
			$html = $this->load->view('unit_processes_water/client/pdf_view_by_uf', $view_data, true);
		} else {
			$html = $this->load->view('unit_processes_water/client/pdf_view', $view_data, true);
		}
		...
		$pdf_file_name = $client_info->sigla."_".$project_info->sigla."_".lang("unit_processes_water_pdf")."_".date('Y-m-d').".pdf";
	...
	// Versión que recalcula los valores para las huellas
	function get_pdf_bak(){
		...
		// COLUMNAS HUELLAS FIJAS
		$array_columnas[] = lang("water_in_product")."<br /> (m3 agua EQ)";
		$array_columnas[] = lang("consumed_water")."<br /> (m3 agua EQ)";
	...
	private function _make_row_pdf($data, $uf_data, $start_date, $end_date){
        ...
        //$row_data[] =  modal_anchor(get_uri("unit_processes_carbon/view/" . $data->id), "<i class='fa fa-eye'></i>", array("title" => lang('view_functional_unit'), "data-post-id" => $data->id));
        return $row_data;
	...
	function get_unit_processes(){
		...
		$huellas_fijas = ',{"title":"'.lang("water_in_product").'<br /> ('.$unidad_volumen.' agua EQ)", "class": "text-right dt-head-center"}';
		$huellas_fijas .= ',{"title":"'.lang("consumed_water").'<br /> ('.$unidad_volumen.' agua EQ)", "class": "text-right dt-head-center"}';
		$columnas = $this->Project_rel_footprints_model->get_footprints_of_project_json($id_proyecto, $options_footprint_ids);
		$view_data["columnas"] = $columnas.$huellas_fijas;
		//$huellas = $this->Project_rel_footprints_model->get_footprints_of_project($id_proyecto, $options_footprint_ids)->result();
		...
		$alias = $this->Categories_alias_model->get_dropdown_list(array("alias"), 'id_categoria', array("id_cliente" => $id_cliente));
		$categorias = $this->Categories_model->get_dropdown_list(array("nombre"), 'id');
		$array_categorias = array();
		foreach($categorias as $id => $categoria) {
			$array_categorias[$id] = isset($alias[$id]) ? $alias[$id] : $categoria;
		}
		$view_data["categorias"] = $array_categorias;
		...
		if($client_info->habilitado){
			if($unidad_funcional->id){
				echo $this->load->view("unit_processes_water/client/unit_process_by_date", $view_data, TRUE);
			} else {
				echo $this->load->view("unit_processes_water/client/unit_processes_by_date", $view_data, TRUE);
			}
		}else{
			$this->session->sess_destroy();
			redirect('signin/index/disabled');
