...
class Compromises_compliance_evaluation extends MY_Controller {
    ...
    function index() {
		...
		$evaluados = $this->Evaluated_compromises_model->get_evaluated_related_to_project_compromise($id_proyecto)->result_array();
		...
		$id_compromiso_proyecto = $this->Compromises_model->get_one_where(array("id_proyecto" => $id_proyecto, "deleted" => 0))->id;
		$compromisos = $this->Values_compromises_model->get_all_where(array("id_compromiso" => $id_compromiso_proyecto, "deleted" => 0))->result_array();
        ...
        $this->template->rander("compromises_compliance_evaluation/index", $view_data);
	...
	function modal_form() {
        ...
        if ($compliance_evaluation_id) {
		    ...
		    $compliance_evaluation_info = $this->Compromises_compliance_evaluation_model->get_one($compliance_evaluation_id);
			//$evaluaciones_historicas = $this->Compromises_compliance_evaluation_model->get_all_where(array("id_valor_compromiso" => $compliance_evaluation_info->id_valor_compromiso, "id_evaluado" => $compliance_evaluation_info->id_evaluado, "deleted" => 0))->result();
			$evaluaciones_historicas = $this->Compromises_compliance_evaluation_model->get_all_where_order_by_date_desc(array("id_valor_compromiso" => $compliance_evaluation_info->id_valor_compromiso, "id_evaluado" => $compliance_evaluation_info->id_evaluado))->result();
            ...
            if ($compliance_evaluation_info) {
				...
				$nombre_compromiso = $this->Values_compromises_model->get_one($compliance_evaluation_info->id_valor_compromiso)->nombre_compromiso;
				$nombre_evaluado = $this->Evaluated_compromises_model->get_one($compliance_evaluation_info->id_evaluado)->nombre_evaluado;
				...
				*/
				...
				$archivos_evidencia = $this->Compromises_compliance_evidences_model->get_all_where(array("id_evaluacion_cumplimiento_compromiso" => $compliance_evaluation_info->id, "deleted" => 0))->result_array();
				...
				if($archivos_evidencia){
					...
					foreach($archivos_evidencia as $evidencia){
						...
						$html_archivos_evidencia .= anchor(get_uri("compromises_compliance_evaluation/download_file/".$compliance_evaluation_info->id. "/" . $evidencia["id"]), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));
				...
				$this->load->view('compromises_compliance_evaluation/compliance_evaluation/modal_form', $view_data);
			...
			$nombre_evaluado = $this->Evaluated_compromises_model->get_one($id_evaluado)->nombre_evaluado;
	...
	function save() {
		...
		if($compliance_evaluation_id){
			...
			if($id_evidencia_eliminar){
				...
				foreach($id_evidencia_eliminar as $id_evidencia){
					...
					$id_valor_compromiso = $this->Compromises_compliance_evaluation_model->get_one($compliance_evaluation_id)->id_valor_compromiso;
					$id_evaluado = $this->Compromises_compliance_evaluation_model->get_one($compliance_evaluation_id)->id_evaluado;
					$id_compromiso_proyecto = $this->Values_compromises_model->get_one($id_valor_compromiso)->id_compromiso;
					
					$id_proyecto = $this->Compromises_model->get_one($id_compromiso_proyecto)->id_proyecto;
		...
		if($compliance_evaluation_id){ //edit
			...
			if($id_evaluacion_historica){
				...
				/*
				$evaluacion_historica = $this->Compromises_compliance_evaluation_model->get_one($id_evaluacion_historica);
				$fecha_evaluacion = $data_compliance_evaluation["fecha_evaluacion"];
				if($fecha_evaluacion !== $evaluacion_historica->fecha_evaluacion){
					$evaluaciones = $this->Compromises_compliance_evaluation_model->get_all_where(array("id_evaluado" => $id_evaluado, "id_valor_compromiso" => $id_valor_compromiso, "deleted" => 0))->result_array();	
				...
				*/
				...
				$save_id = $this->Compromises_compliance_evaluation_model->save($data_compliance_evaluation, $id_evaluacion_historica);
				...
				$save_id = $this->Compromises_compliance_evaluation_model->save($data_compliance_evaluation);
        ...
        if ($save_id) {
			...
			$id_valor_compromiso = $this->Compromises_compliance_evaluation_model->get_one($save_id)->id_valor_compromiso;
			$id_evaluado = $this->Compromises_compliance_evaluation_model->get_one($save_id)->id_evaluado;
			$id_compromiso_proyecto = $this->Values_compromises_model->get_one($id_valor_compromiso)->id_compromiso;
			...
			if($file){
				...
				//$archivo_subido = move_temp_file($file, "files/evaluaciones_compromisos/evaluacion_".$save_id."/");

				$evaluacion = $this->Compromises_compliance_evaluation_model->get_one($save_id);
				$valor_compromiso = $this->Values_compromises_model->get_one($evaluacion->id_valor_compromiso);
				$matriz_compromiso = $this->Compromises_model->get_one($valor_compromiso->id_compromiso);
	...
	function list_data($id_compromiso_proyecto, $id_evaluado, $id_valor_compromiso) {
		...
		$list_data = $this->Compromises_compliance_evaluation_model->get_details($options);
	...
	private function _make_row($data) {
		...
		$nombre_compromiso = $this->Values_compromises_model->get_one($data->id_valor_compromiso)->nombre_compromiso;
		$nombre_evaluado = $this->Evaluated_compromises_model->get_one($data->id_evaluado)->nombre_evaluado;
		...
		$row_data[] = modal_anchor(get_uri("compromises_compliance_evaluation/view"), "<i class='fa fa-eye'></i>", array("class" => "edit", "title" => lang('view_compliance_evaluation'), "data-post-id_evaluacion" => $data->id))
				    . modal_anchor(get_uri("compromises_compliance_evaluation/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_compliance_evaluation'), "data-post-id_evaluacion" => $data->id))
	...
	function view() {
        ...
        if ($compliance_evaluation_id) {
            ...
            if ($compliance_evaluation_info) {
				...
				$evaluaciones_evaluado_compromiso = $this->Compromises_compliance_evaluation_model->get_all_where_order_by_date_desc(array("id_evaluado" => $compliance_evaluation_info->id_evaluado, "id_valor_compromiso" => $compliance_evaluation_info->id_valor_compromiso))->result();
				$evaluations_dropdown = array();
				...
				$archivos_evidencia = $this->Compromises_compliance_evidences_model->get_all_where(array("id_evaluacion_cumplimiento_compromiso" => $compliance_evaluation_info->id, "deleted" => 0))->result();
				...
				if($archivos_evidencia){
					foreach($archivos_evidencia as $evidencia){
						...
						$html_archivos_evidencia .= anchor(get_uri("compromises_compliance_evaluation/download_file/".$compliance_evaluation_id. "/" . $evidencia->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));
						$html_archivos_evidencia .= '</td>';
						$html_archivos_evidencia .= '</tr>';
						$html_archivos_evidencia .= '</tbody>';
						$html_archivos_evidencia .= '</table>';
				...
				$this->load->view('compromises_compliance_evaluation/compliance_evaluation/view', $view_data);
	...
	function view_evidences() {
		...
		$evaluacion = $this->Compromises_compliance_evaluation_model->get_one($compliance_evaluation_id);
		$evaluaciones_evaluado_compromiso = $this->Compromises_compliance_evaluation_model->get_all_where_order_by_date_desc(array("id_evaluado" => $evaluacion->id_evaluado, "id_valor_compromiso" => $evaluacion->id_valor_compromiso))->result();
		$evaluations_dropdown = array();
		foreach($evaluaciones_evaluado_compromiso as $eec){
			$evaluations_dropdown[$eec->id] = lang("evaluation_with_date") . " " . get_date_format($eec->fecha_evaluacion, $this->session->project_context);
		}
        ...
        if ($compliance_evaluation_id) {
            ...
            if ($archivos_evidencia) {
				...
				$this->load->view('compromises_compliance_evaluation/compliance_evaluation/view_evidences', $view_data);
	...
	function get_compromises_of_evaluated(){
		...
		$valores_compromisos_evaluado = $this->Values_compromises_model->get_all_where(array("id_compromiso" => $id_matriz_compromiso_evaluado, "deleted" => 0))->result_array();
	...
	function create_compliance_evidence_folder($id_evaluacion) {
		...
		$evaluacion = $this->Compromises_compliance_evaluation_model->get_one($id_evaluacion);
		$valor_compromiso = $this->Values_compromises_model->get_one($evaluacion->id_valor_compromiso);
		$matriz_compromiso = $this->Compromises_model->get_one($valor_compromiso->id_compromiso);
		$id_proyecto = $matriz_compromiso->id_proyecto;
		$id_cliente = $this->Projects_model->get_one($id_proyecto)->client_id;
	...
	function delete_file() {
		...
		$id_valor_compromiso = $this->Compromises_compliance_evaluation_model->get_one($id_evaluacion)->id_valor_compromiso;
		$id_evaluado = $this->Compromises_compliance_evaluation_model->get_one($id_evaluacion)->id_evaluado;
		$id_compromiso_proyecto = $this->Values_compromises_model->get_one($id_valor_compromiso)->id_compromiso;
	...
	function get_form_fields_of_evaluation(){
		...
		$id_proyecto = $this->session->project_context;
		$evaluacion = $this->Compromises_compliance_evaluation_model->get_one($id_evaluacion);
		$compromiso = $this->Values_compromises_model->get_one($evaluacion->id_valor_compromiso);
		$evaluado = $this->Evaluated_compromises_model->get_one($evaluacion->id_evaluado);
		$id_cliente = $this->Projects_model->get_one($id_proyecto)->client_id;
		...
		/*
		var_dump("select_evaluado: " . $select_evaluado);
		var_dump("select_valor_compromiso: " . $select_valor_compromiso);
		...
		*/
		...
		if($id_evaluacion){
			...
			$archivos_evidencia = $this->Compromises_compliance_evidences_model->get_all_where(array("id_evaluacion_cumplimiento_compromiso" => $id_evaluacion, "deleted" => 0))->result_array();
			...
			if($archivos_evidencia){
				...
				foreach($archivos_evidencia as $evidencia){
					...
					$html_archivos_evidencia .= '<table id="table_delete_'.$evidencia["id"].'" class="table_delete"><thead><tr><th></th></tr></thead>';
					$html_archivos_evidencia .= '<tbody><tr><td class="option text-center">';
					$html_archivos_evidencia .= anchor(get_uri("compromises_compliance_evaluation/download_file/".$id_evaluacion. "/" . $evidencia["id"]), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));
			...
			$compromiso = $this->Values_compromises_model->get_one($id_valor_compromiso);
