...
class Consolidated_impacts extends MY_Controller {
    ...
    public function index() {
		...
		// ARREGLO DE SUBPROYECTOS QUE SE MOSTRARÁN EN LOS GRÁFICOS AL HACER CLICK EN UNA COLUMNA DE CATEGORÍA (TERCER NIVEL)
		...
		$campos_unidad_consumo_cliente = $this->Forms_model->get_details_client(array("id_cliente" => $id_cliente, "flujo" => "Consumo"))->result();
		$campos_unidad_residuo_cliente = $this->Forms_model->get_details_client(array("id_cliente" => $id_cliente, "flujo" => "Residuo"))->result();
		...
		$view_data['array_grafico_consumos_volumen_data'] = $this->generar_datos_grafico($array_id_materiales_valores_volumen, 'Consumo', $id_cliente, $years, $array_subprojects);
		...
		$view_data['array_grafico_consumos_masa_data'] = $this->generar_datos_grafico($array_id_materiales_valores_masa, 'Consumo', $id_cliente, $years, $array_subprojects);
		...
		$view_data['array_grafico_consumos_energia_data'] = $this->generar_datos_grafico($array_id_materiales_valores_energia, 'Consumo', $id_cliente, $years, $array_subprojects);
		...
		$view_data['array_grafico_residuos_volumen_data'] = $this->generar_datos_grafico($array_id_materiales_valores_volumen_residuo, 'Residuo', $id_cliente, $years, $array_subprojects);
		...
		$view_data['array_grafico_residuos_masa_data'] = $this->generar_datos_grafico($array_id_materiales_valores_masa_residuo, 'Residuo', $id_cliente, $years, $array_subprojects);
		// FIN DATOS PARA GRÁFICO Y TABLA RESIDUOS MASA
		// FIN RESIDUO

		$this->template->rander("consolidated_impacts/client_dashboard", $view_data);
				...
				foreach($materiales_rel_categorias as $mat_rel_cat){
					...
					foreach($years as $year){
						foreach($array_subprojects as $id_subproject => $name_subproject){
							$array_id_materiales_valores[$mat_rel_cat->id_material][$mat_rel_cat->id_categoria][$year][$id_subproject] = 0;
						}
					}
				}
				...
				foreach($materiales_rel_categorias as $mat_rel_cat){
					...
					foreach($elementos_form as $ef){
						...
						if($id_unidad == $id_unidad_configuración){
							...
							$array_id_materiales_valores[$mat_rel_cat->id_material][$mat_rel_cat->id_categoria][$agno][$id_sucursal] += $valor;
							...
							$array_id_materiales_valores[$mat_rel_cat->id_material][$mat_rel_cat->id_categoria][$agno][$id_sucursal] += $valor * $valor_transformacion;
	...
	function generar_datos_grafico(array $array_id_materiales_valores_por_flujo, string $flujo, int $id_cliente, array $years, array $array_subprojects){
		...
		foreach($years as $year){
			...
			// AQUÍ COLOCAR EL FOREACH DE PROYECTOS
		...
		foreach($years as $year){
			...
			foreach ($array_id_materiales_valores_por_flujo as $id_material => $array_id_categorias_valores_por_flujo){
				foreach ($array_id_categorias_valores_por_flujo as $id_categoria => $arreglo_valores_by_year){
					...
					foreach($array_subprojects as $id_subproject => $name_subproject){
						$valor_subproject = $arreglo_valores_by_year[$year][$id_subproject];
						$serie['data'][] = array($name_subproject, $valor_subproject);
					}
