...
class Functional_units extends MY_Controller {
    ...
    function index() {
		...
		if ($this->login_user->user_type === "staff") {
			...
			// ARREGLOS PARA DATOS
			$view_data["calculos"] = $this->Calculation_model->get_calculos($id_proyecto, $client_info->id, NULL, NULL, NULL)->result();
	...
	function get_functional_units(){
		...
		$view_data["calculos"] = $this->Calculation_model->get_calculos($id_proyecto, $client_info->id, NULL, $start_date, $end_date)->result();
		
		if($client_info->habilitado){
			//$this->template->rander("functional_units/client/index", $view_data);
			echo $this->load->view("functional_units/client/functional_units_by_date2", $view_data, TRUE);
		}else{
			$this->session->sess_destroy();
			redirect('signin/index/disabled');
		}
