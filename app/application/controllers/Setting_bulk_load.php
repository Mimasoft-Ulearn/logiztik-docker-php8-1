...
class Setting_bulk_load extends MY_Controller {
	...
	function get_excel_template_of_form() {
		...
		if($info_formulario->id_tipo_formulario == 1){
			...
			$doc->getActiveSheet()->setCellValue($this->getNameFromNumber($columna).'1', lang('branch_office'));
			$columna++;
			...
			if($info_formulario->flujo == "Residuo"){
				...
				// Si el proyecto esta ubicado en la región metropolitana
				if($info_proyecto->in_rm){
					
					$doc->getActiveSheet()->setCellValue($this->getNameFromNumber($columna).'1', lang('carrier_rut'));
					$columna++;
					
					$doc->getActiveSheet()->setCellValue($this->getNameFromNumber($columna).'1', lang('patent'));
					$columna++;
			...
			// COLUMNA SUCURSAL
			$sucursales = $this->Subprojects_model->get_details(array("id_proyecto" => $this->session->project_context))->result();
			$array_sucursales = array();
			foreach($sucursales as $sucursal){
				$array_sucursales[] = $sucursal->nombre;
			}

			// GUARDO OPCIONES DE SELECT SUCURSALES EN HOJA OPCIONES (ETIQUETAS)
			$doc->setActiveSheetIndex(1);
			$fila_opcion = 1;
			foreach($array_sucursales as $sucursal){
				$doc->getActiveSheet()->setCellValue($this->getNameFromNumber($columna_opciones).$fila_opcion, $sucursal);
				$fila_opcion++;
			}
			
			$doc->setActiveSheetIndex(0);

			if(count($array_sucursales)){
				$objValidation = $doc->getActiveSheet()->getCell($this->getNameFromNumber($columna).'2')->getDataValidation();     
				$objValidation->setType(PHPExcel_Cell_DataValidation::TYPE_LIST);     
				$objValidation->setErrorStyle(PHPExcel_Cell_DataValidation::STYLE_INFORMATION);     
				$objValidation->setAllowBlank(false);
				...
				$cantidad_sucursales = count($array_sucursales);
	
				$col_opt = $this->getNameFromNumber($columna_opciones);
				
				$objValidation->setFormula1('options!$'.$col_opt.'$1:$'.$col_opt.'$'.$cantidad_sucursales);
				$doc->getActiveSheet()->setCellValue($this->getNameFromNumber($columna).'2', $array_sucursales[0]);
			}
			...
			if($info_formulario->flujo == "Consumo"){
				...
				if(count($array_tipo_tratamiento)){
					$objValidation = $doc->getActiveSheet()->getCell($this->getNameFromNumber($columna).'2')->getDataValidation();
					$objValidation->setType(PHPExcel_Cell_DataValidation::TYPE_LIST);     
					$objValidation->setErrorStyle(PHPExcel_Cell_DataValidation::STYLE_INFORMATION);     
					$objValidation->setAllowBlank(false);
			...
			if($info_formulario->flujo == "Residuo"){
				...
				if($info_proyecto->in_rm){
					...
					// COLUMNA RUT TRANSPORTISTA
					$doc->setActiveSheetIndex(0);

					$doc->getActiveSheet()->setCellValue($this->getNameFromNumber($columna).'2', "1111111-1");
					$columna++;
				...
				if(count($array_waste_transport_companies)){
					$objValidation = $doc->getActiveSheet()->getCell($this->getNameFromNumber($columna).'2')->getDataValidation();     
					$objValidation->setType(PHPExcel_Cell_DataValidation::TYPE_LIST);     
					$objValidation->setErrorStyle(PHPExcel_Cell_DataValidation::STYLE_INFORMATION);     
					$objValidation->setAllowBlank(false);
				...
				if(count($array_waste_receiving_companies)){
					$objValidation = $doc->getActiveSheet()->getCell($this->getNameFromNumber($columna).'2')->getDataValidation();     
					$objValidation->setType(PHPExcel_Cell_DataValidation::TYPE_LIST);     
					$objValidation->setErrorStyle(PHPExcel_Cell_DataValidation::STYLE_INFORMATION);     
					$objValidation->setAllowBlank(false);
    ...
    function save() {
		...
		if($archivo_subido){
			...
			if($id_tipo_formulario == 1){// SI ES REGISTRO AMBIENTAL
				...
				if(lang('branch_office') == $worksheet->getCell($letra_columna.'1')->getValue()){
						$html .= '<th>'.$worksheet->getCell($letra_columna.'1')->getValue().'</th>';
				}else{
					$html .= '<th class="error app-alert alert-danger">'.$worksheet->getCell($letra_columna.'1')->getValue().' '.$msg_columna.'</th>';
					$num_errores++;
				...
				if($info_formulario->flujo == "Residuo"){
					...
					// Si el proyecto se ubica en la región metropolitana
					if($info_proyecto->in_rm){
						// CAMPO RUT TRANSPORTISTA
						if(lang('carrier_rut') == $worksheet->getCell($this->getNameFromNumber($cont).'1')->getValue()){
						...
						// CAMPO PATENTE
						if(lang('patent') == $worksheet->getCell($this->getNameFromNumber($cont).'1')->getValue()){
							$html .= '<th>'.$worksheet->getCell($this->getNameFromNumber($cont).'1')->getValue().'</th>';
						}else{
							$html .= '<th class="error app-alert alert-danger">'.$worksheet->getCell($this->getNameFromNumber($cont).'1')->getValue().' '.$msg_columna.'</th>';
							$num_errores++;
				...
				for($row = 2; $row <= $lastRow; $row++){
					...
					$sucursal_excel = $worksheet->getCell($letra_columna.$row)->getValue();
					
					if(in_array($sucursal_excel, $array_sucursales)){
						$html .= '<td>'.$sucursal_excel.'</td>';
					}else{
						$html .= '<td class="error app-alert alert-danger">'.$sucursal_excel.' '.$msg_formato.'</td>';
						$num_errores++;
					}
					...
					if($info_formulario->flujo == "Residuo"){
						...
						if($info_proyecto->in_rm){
							// RUT TRANSPORTISTA
							$carrier_rut = $worksheet->getCell($this->getNameFromNumber($cont).$row)->getValue();
							if(strlen(trim($carrier_rut)) > 0){
								$html .= '<td>'.$carrier_rut.'</td>';
							}else{
								$html .= '<td class="error app-alert alert-danger">'.$carrier_rut.' '.$msg_obligatorio.'</td>';
								$num_errores++;
							}
							$cont++;

							// PATENTE
							$patent_excel = $worksheet->getCell($this->getNameFromNumber($cont).$row)->getValue();
							$array_patents = array();
							$patents = $this->Patents_model->get_all_where(array(
								"id_client" => $id_cliente,
								"id_project" => $id_proyecto,
							...
							if(in_array($patent_excel, $array_patents)){
								$html .= '<td>'.$patent_excel.'</td>';
							}else{
								$html .= '<td class="error app-alert alert-danger">'.$patent_excel.' '.$msg_formato.'</td>';
								$num_errores++;
							}
							$cont++;
	...
	function bulk_load($id_cliente, $id_proyecto, $id_formulario, $archivo_subido){
		...
		if($formulario->id_tipo_formulario == 1){// SI ES REGISTRO AMBIENTAL
			...
			for($row = 2; $row <= $lastRow; $row++){
				...
				// CAMPO SUCURSAL
				$letra_columna = $this->getNameFromNumber($cont);
				$label_sucursal = $worksheet->getCell($letra_columna.$row)->getValue();
				$cont++;

				$sucursal = $this->Subprojects_model->get_one_where(array("nombre" => $label_sucursal, "deleted" => 0));
				if($sucursal->id){
					$array_json["id_sucursal"] = (int)$sucursal->id;
				}
				...
				if($info_formulario->flujo == "Residuo"){
					...
					// Si el proyecto esta ubicado en la región metropolitana
					if($info_proyecto->in_rm){
						//"carrier_rut":"9753108","id_patent":"8","container":"another_container","id_waste_receiving_company":"5"
						...
						// CAMPO PATENTE
						$patent = $worksheet->getCell($this->getNameFromNumber($cont).$row)->getValue();
						$patente = $this->Patents_model->get_one_where(array(
							"id_client" => $id_cliente,
							"id_project" => $id_proyecto,
							"patent" => $patent
				...
				if($info_formulario->flujo == "Consumo"){
					...
					$array_json["carrier_rut"] = $carrier_rut;
	...
	function get_fields_of_fixed_feeders($id_formulario) {
        ...
        if($id_formulario == "waste_transport_companies"){
			...
			$campo_2->nombre = lang("company_rut");
			...
			$campo_4->nombre = lang("receiving_establishment_treatment");
			...
			$campo_9->nombre_columna = "province";
			$campos_formulario[8] = $campo_9;

			$campo_10 = new stdClass();
			$campo_10->id_tipo_campo = 1;
			$campo_10->nombre = lang("commune");
			$campo_10->nombre_columna = "commune";
