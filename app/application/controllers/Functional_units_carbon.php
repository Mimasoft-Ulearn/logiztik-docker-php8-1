...
class Functional_units_carbon extends MY_Controller {
    ...
    function index() {
		...
		if ($this->login_user->user_type === "staff") {
			...
			$footprints = $this->Footprints_model->get_footprints_of_methodology(3)->result(); // Metodología con id 3: Iso 1404
			$footprint_ids = array();
			...
			$last = count($array_periodos) ? array_keys($array_periodos)[11] : null;
			...
			// $view_data["calculos"] = $this->Calculation_model->get_calculos($from, $client_info->id, $to)->result();

			if($client_info->habilitado){
				$this->template->rander("functional_units_carbon/client/index", $view_data);
			}else{
	...
	function get_pdf(){
		...
		$html = $this->load->view('functional_units_carbon/client/pdf_view', $view_data, true);
	...
	function get_functional_units(){
		...
		if($id_unidad_funcional){
			...
			//echo '<pre>'; var_dump($view_data["calculos"]);
		}else{
		...
		if($client_info->habilitado){
			...
			if($unidad_funcional->id){
				echo $this->load->view("functional_units_carbon/client/functional_unit_by_date2", $view_data, TRUE);
			} else {
				echo $this->load->view("functional_units_carbon/client/functional_units_by_date2", $view_data, TRUE);
			}
