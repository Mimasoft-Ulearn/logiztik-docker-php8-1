...
class Footprint_format extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("footprint_format/index");
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Footprint_format_model->get_one($id);
		
		$view_data["metodologias_disponibles"] = $this->Methodology_model->get_all()->result_array();
		
		if($id){
           $view_data["metodologias_de_fh"] = $this->Methodology_model->get_methodologies_of_fh($id)->result_array();
        }
		
        $this->load->view('footprint_format/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if($id){
            ...
            $save_id = $this->Footprint_format_model->save($data, $id);
			$delete_bd_rel_meth = $this->Footprint_format_model->delete_fh_rel_methodology($id);
			if($delete_bd_rel_meth){
				if($metodologias_multiselect){
					foreach($metodologias_multiselect as $id_metodologia){
						$data_b_rel_m["id_fh"] = $id;
						...
						$save_data_b_rel_m = $this->FH_rel_methodology_model->save($data_b_rel_m);
					}
            ...
            $save_id = $this->Footprint_format_model->save($data); 
			if($metodologias_multiselect){
				foreach($metodologias_multiselect as $id_metodologia){
    ...
    function list_data() {
        ...
        $this->access_only_allowed_members();
        $list_data = $this->Footprint_format_model->get_details()->result();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Footprint_format_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        $row_data = array($data->id,
            modal_anchor(get_uri("footprint_format/view/" . $data->id), $data->nombre, array("title" => lang('view_footprint_format'))),
        );
        ...
        $row_data[] = modal_anchor(get_uri("footprint_format/view/" . $data->id), "<i class='fa fa-eye'></i>", array("title" => lang('view_footprint_format'))) .
        modal_anchor(get_uri("footprint_format/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_footprint_format'), "data-post-id" => $data->id))

                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_footprint_format'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("footprint_format/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($id_fh = 0) {

        if ($id_fh) {
            $options = array("id" => $id_fh);
            $model_info = $this->Footprint_format_model->get_details($options)->row();
			
            if ($model_info) {
                $view_data['model'] = $model_info;
                $view_data["metodologias_de_bd"] = $this->Methodology_model->get_methodologies_of_fh($id_fh)->result_array();
                $this->load->view('footprint_format/view', $view_data);
            } else {
                show_404();
            }
