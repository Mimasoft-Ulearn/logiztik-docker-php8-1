...
class Sinader_code extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("sinader_code/index", $view_data);
    ...
    function modal_form() {
		...
		$view_data['model_info'] = $this->Sinader_code_model->get_one($id);
        ...
        $this->load->view('sinader_code/modal_form', $view_data);
    ...
    function save() {
        ...
        $data = array(
			...
			"ler_code" => trim($this->input->post('ler_code')),
        );
        ...
        // Validamos que previamente no exista un code para cliente, proyecto y categoria ingresado
		...
		if ($this->Sinader_code_model->is_code_exists($array_validacion, $id)) {
			echo json_encode(array("success" => false, 'message' => lang('duplicate_sinader_code_in_client')));
			exit();
		}
		
        $save_id = $this->Sinader_code_model->save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Sinader_code_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Sinader_code_model->delete($id)) {
    ...
    function list_data() {
        ...
        $list_data = $this->Sinader_code_model->get_details($options)->result();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Sinader_code_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        $row_data = array(
            ...
            $data->ler_code ? $data->ler_code : "-"
        );

        $row_data[] = modal_anchor(get_uri("sinader_code/view/" . $data->id), "<i class='fa fa-eye'></i>", array("title" => lang('view_sinader_code'))) .
        modal_anchor(get_uri("sinader_code/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_sinader_code'), "data-post-id" => $data->id))
        . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_sinader_code'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("sinader_code/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($id = 0) {
        ...
        if ($id) {
            $options = array("id" => $id);
            $model_info = $this->Sinader_code_model->get_details($options)->row();
            if ($model_info){
                ...
                $this->load->view('sinader_code/view', $view_data);
            } else {
                show_404();
            }
        } else {
