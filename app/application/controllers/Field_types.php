...
class Field_types extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander("field_types/index");
    }
	
	/* list of field types, prepared for datatable  */
    ...
    function list_data() {
		...
		$options = array();
        $list_data = $this->Field_types_model->get_details($options)->result();
...
//NOT USED 
    ...
    /* prepare a row of field types list table */
    ...
    private function _make_row($data) {
        ...
        if ($data->descripcion) {
            $note_link = modal_anchor(get_uri("projects/timer_note_modal_form/"), "<i class='fa fa-comment p10'></i>", array("class" => "edit text-muted", "title" => $data->descripcion, "data-modal-title" => lang("note"), "data-post-id" => $data->id));
        }*/
		
		if($data->habilitado){
			$habilitado = "<i class='fa fa-check p10'></i>";
		}else{
			$habilitado = "<i class='fa fa-times p10'></i>";
		}
		

        $row_data = array($data->id,
            //anchor(get_uri("field_types/view/" . $data->id), $data->company_name),
			$data->nombre,
            //$note_link,
