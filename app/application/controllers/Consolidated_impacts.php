...
class Consolidated_impacts extends MY_Controller {
    ...
    public function index() {
		...
		$year_changed = $this->input->post("year_changed");
        $selected_year = $this->input->post("selected_year");
        $current_year = date("Y");
        $view_data["selected_year"] = $selected_year ? $selected_year : $current_year;

		$year_2 = $selected_year ? $selected_year : $current_year;
        $year_1 = $year_2 -1;
        $years = array((string)$year_1, (string)$year_2);
		...
		// PARA MOSTRAR LOS RESULTADOS DE HUELLAS SOLO DEL AÑO EN CURSO O EL AÑO SELECCIONADO EN EL FILTRO
		// $view_data["first_date_current_year"] = date('Y-01-01');
		// $view_data["last_date_current_year"] = date('Y-12-31');
		$view_data["first_date_current_year"] = date("$year_2-01-01");
		$view_data["last_date_current_year"] = date("$year_2-12-31");
		...
		if($year_changed){
            $this->load->view("consolidated_impacts/client_dashboard", $view_data);
        } else {
            $this->template->rander("consolidated_impacts/index", $view_data);
        }
