...
class Relationship extends MY_Controller {
    ...
    private function _make_criterio_row($data) {
		...
		if(isset($data->tipo_by_criterio)){
			...
			if($jsno_data["id_campo_sp"] == "tipo_tratamiento" ){
				...
				$campo_sp = lang('type');
			} elseif($jsno_data["id_campo_sp"] == "month"){
				$campo_sp = lang('month');
			} elseif($jsno_data["id_campo_sp"] == "id_sucursal"){
			...
			if($jsno_data["id_campo_pu"] == "tipo_tratamiento" ){
				...
				$campo_pu = lang('type');
			} elseif($jsno_data["id_campo_pu"] == "month"){
				$campo_pu = lang('month');
			} elseif($jsno_data["id_campo_pu"] == "id_sucursal"){
			...
			if($jsno_data["id_campo_fc"] == "tipo_tratamiento" ){
				...
				$campo_fc = lang('type');
			} elseif($jsno_data["id_campo_fc"] == "month"){
				$campo_fc = lang('month');
			} elseif($jsno_data["id_campo_fc"] == "id_sucursal"){
    ...
    function add_rule() {
		...
		if($rule_id){
			...
			$campos["id_sucursal"] = lang('branch_office');
    ...
    function get_fields_of_form(){
		...
		if($id_formulario){
			...
			if($form->flujo == "Residuo"){
				...
				$array_campos_select[] = array("id" => "month", "text" => ''.lang('month').'');
			}
			...
			$array_campos_select[] = array("id" => "id_sucursal", "text" => ''.lang('branch_office').'');
    ...
    function save_criterio() {
		...
		if(in_array($this->input->post('subproject_rule'), array('tipo_tratamiento','type_of_origin_matter','type_of_origin', 'default_type', 'month', 'id_sucursal'))){
			$id_campo_sp_js = $this->input->post('subproject_rule');
		} else {
			$id_campo_sp_js = NULL;
		}
		
		if(in_array($this->input->post('unit_processes_rule'), array('tipo_tratamiento','type_of_origin_matter','type_of_origin', 'default_type', 'month', 'id_sucursal'))){
			$id_campo_pu_js = $this->input->post('unit_processes_rule');
		} else {
			$id_campo_pu_js = NULL;
		}

		if(in_array($this->input->post('fc_rule'), array('tipo_tratamiento','type_of_origin_matter','type_of_origin', 'default_type', 'month', 'id_sucursal'))){
	...
	function get_rule_options(){
			...
			if(isset($j_data->id_campo_sp)){
				...
				if($flujo_formulario_criterio == "Residuo"){
					...
					if($j_data->id_campo_sp == "tipo_tratamiento"){
						...
						$campos_fijos = $this->Tipo_tratamiento_model->get_details()->result();
					...
					}elseif($j_data->id_campo_sp == "month"){
				...
				if($j_data->id_campo_sp == "id_sucursal") {
					$opciones_campo_sp = array();
					$sucursales = $this->Subprojects_model->get_all_where(array(
						"id_proyecto" => $id_proyecto,
						"deleted" => 0
					))->result();
		...
		if(isset($criterio->tipo_by_criterio) && json_decode($criterio->tipo_by_criterio)->id_campo_pu){
			...
			if($flujo_formulario_criterio == "Residuo"){
				...
				} /* elseif($j_data->id_campo_pu == "month"){
			...
			if($j_data->id_campo_pu == "id_sucursal") {
				$opciones_campo_pu = array();
				$sucursales = $this->Subprojects_model->get_all_where(array(
					"id_proyecto" => $id_proyecto,
					"deleted" => 0
    ...
    private function _make_calculo_row($data) {
		...
		*/
		...
		$calculo_info = $this->Calculation_model->get_details(array("id" => $data->id))->row();
		$flujo_formulario_criterio = $form->flujo;
		...
		if((isset($id_campo_fc_js)) /*|| ($id_campo_fc_js == 1)*/){
			...
			if($flujo_formulario_criterio == "Residuo"){
				if($id_campo_fc_js == "tipo_tratamiento"){
					$criterio_fc = $calculo_info->criterio_fc ? $this->Tipo_tratamiento_model->get_one($calculo_info->criterio_fc)->nombre : "-";
				}elseif($id_campo_fc_js == "month"){
					$criterio_fc = $calculo_info->criterio_fc ? number_to_month($calculo_info->criterio_fc) : "-";
				}
			}

			if($flujo_formulario_criterio == "Consumo"){
				$data_tipo_origen = json_decode($form->tipo_origen);
				if($data_tipo_origen->type_of_origin == "1"){ // id 1: matter
					$criterio_fc = $calculo_info->criterio_fc ? lang($this->EC_Types_of_origin_matter_model->get_one($calculo_info->criterio_fc)->nombre) : "-";
				}
				if($data_tipo_origen->type_of_origin == "2"){ // id 2: energy
					$criterio_fc = $calculo_info->criterio_fc ? lang($this->EC_Types_of_origin_model->get_one($calculo_info->criterio_fc)->nombre) : "-";
				}
			}
			
			if($flujo_formulario_criterio == "No Aplica"){
				$criterio_fc = $calculo_info->criterio_fc ? lang($this->EC_Types_no_apply_model->get_one($calculo_info->criterio_fc)->nombre) : "-";
			}

			if($id_campo_fc_js == "id_sucursal") {
				$criterio_fc = $calculo_info->criterio_fc ? $this->Subprojects_model->get_one($calculo_info->criterio_fc)->nombre : "-";
			}
			...
			if($es_mantenedora["mantenedora"]){
				...
				$id_campo_label = $es_mantenedora["field_label"]; // ID del campo que está seteado como valor para el campo Mantenedora
				$id_campo_value = $es_mantenedora["field_value"]; // ID del campo que está seteado como valor para el campo Mantenedora

				if($id_mantenedora == "waste_transport_companies"){
					$criterio_fc = $calculo_info->criterio_fc ? $this->Fixed_feeder_waste_transport_companies_values_model->get_one($calculo_info->criterio_fc)->company_name : "-";
				}elseif($id_mantenedora == "waste_receiving_companies"){
					$criterio_fc = $calculo_info->criterio_fc ? $this->Fixed_feeder_waste_receiving_companies_values_model->get_one($calculo_info->criterio_fc)->company_name : "-";
				}else{
					$criterio_fc = $calculo_info->criterio_fc ? $calculo_info->criterio_fc : "-";
				}
					...
					if(isset($criterio_data["id_campo_fc"])){	
						...
						if($flujo_formulario_criterio == "Residuo"){
							...
							if($criterio_data["id_campo_fc"] == "tipo_tratamiento"){
								...
								$tipos_tratamientos = $this->Tipo_tratamiento_model->get_details()->result();
								foreach($tipos_tratamientos as $tp){
									$opciones[$tp->id] = $tp->nombre;
								}
			
							}elseif($criterio_data["id_campo_fc"] == "month"){
			
								foreach(CONST_ARRAY_MESES as $month){
									$month_number = month_to_number($month);
									$opciones[$month_number] = $month;
								}
						...
						if($flujo_formulario_criterio == "Consumo"){
							...
							if($data_tipo_origen->type_of_origin == "1"){ // id 1: matter
								...
								foreach($tipos_origen_materia as $campo){
									$opciones[$campo->id] = lang($campo->nombre);
								}
							}
							if($data_tipo_origen->type_of_origin == "2"){ // id 2: energy
								$tipo_origen = $this->EC_Types_of_origin_model->get_one(2);  // id 2: energy
								$opciones[$tipo_origen->id] = lang($tipo_origen->nombre);
							}
						}
						...
						if($criterio_data["id_campo_fc"] == "id_sucursal") {
							$opciones = array();
							$sucursales = $this->Subprojects_model->get_all_where(array(
								"id_proyecto" => $id_proyecto,
								"deleted" => 0
							...
							foreach($sucursales as $sucursal){
								$valor_label = $sucursal->nombre;
								$valor_value = $sucursal->id;
								$opciones[$valor_value] = $valor_label;
							}
						}
	...
	function get_options_of_criterio_fc(){
		...
		if(isset($criterio->tipo_by_criterio)){
			...
			$id_campo_sp_js = $json_data["id_campo_sp"];
			$id_campo_pu_js = $json_data["id_campo_pu"];
		}
			...
			if($flujo_formulario_criterio == "Residuo"){
				...
				if($id_campo_fc_js == "tipo_tratamiento"){
					...
					foreach($tipos_tratamientos as $tipo_tratamiento){
						...
						$valor_value = $tipo_tratamiento->id;
						$opciones[$valor_value] = $valor_label;
					}
					...
					foreach(CONST_ARRAY_MESES as $month){
						$valor_label = $month;
						$valor_value = month_to_number($month);
						$opciones[$valor_value] = $valor_label;
					}
	...
	function view_calculo($calculo_id = 0){		
        if ($calculo_id) {
            ...
            if ($calculo_info) {
				...
				$flujo_formulario_criterio = $form_data->flujo;
				...
				if((isset($id_campo_fc_js)) /*|| ($id_campo_fc_js == 1)*/){
					...
					if($flujo_formulario_criterio == "Consumo"){
						$data_tipo_origen = json_decode($form_data->tipo_origen);
						if($data_tipo_origen->type_of_origin == "1"){ // id 1: matter
							$criterio_fc = $calculo_info->criterio_fc ? lang($this->EC_Types_of_origin_matter_model->get_one($calculo_info->criterio_fc)->nombre) : "-";
						}
						if($data_tipo_origen->type_of_origin == "2"){ // id 2: energy
				...
				$view_data["criterio_fc"] = $criterio_fc;
