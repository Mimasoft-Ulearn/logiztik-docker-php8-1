...
class Forms extends MY_Controller
{
	...
	function modal_form()
	{
		...
		if ($add_type == "multiple" && $last_id) {
			...
			$model_info = $this->Forms_model->get_one($last_id);
			$view_data['model_info'] = $model_info;

			$view_data["form_rel_project"] = $this->Form_rel_project_model->get_one_where(array("id_formulario" => $last_id, "deleted" => 0));
			$view_data["field_rel_form"] = $this->Field_rel_form_model->get_all_where(array("id_formulario" => $last_id, "deleted" => 0))->result_array();

			$form_id = $last_id;
		} else {
			$model_info = $this->Forms_model->get_one($form_id);
			$view_data['model_info'] = $model_info;
		...
		if ($form_id) {
			...
			if (!$view_data['model_info']->fijo) {
				...
				if ($formulario->tipo_origen) {
					...
					$tipos_origen_materia = $this->EC_Types_of_origin_matter_model->get_all_where(
						array(
				...
				$id_proyecto = $this->Form_rel_project_model->get_one_where(
					array(
				...
				$criterio = $this->Rule_model->get_one_where(
					array(
				...
				$calculo = $this->Calculation_model->get_one_where(
					array(
						"id_cliente" => $id_cliente,
						"id_proyecto" => $id_proyecto,
				...
				$view_data["campos_de_proyecto"] = $this->Fixed_fields_model->get_all_where(
					array(
	...
	function save()
	{
		...
		$data_form = array(
			...
			"usuarios" => json_encode($this->input->post('users')),
		);
		...
		if ($form_id) {
			...
			if (!$info_formulario->fijo) {
				...
				$config_factores_transformacion = $this->EC_Client_transformation_factors_config_model->get_all_where(
					array(
						"id_cliente" => $id_cliente,
						"deleted" => 0
					)
				...
				foreach ($array_config_factores_transformacion as $id_categoria_config) {
					if (!in_array($id_categoria_config, $array_categorias_proyectos)) {
						$config = $this->EC_Client_transformation_factors_config_model->get_one_where(
							array(
								"id_cliente" => $id_cliente,
		...
		if ($save_id) {
			...
			echo json_encode(array("success" => true, "data" => $this->_row_data($save_id), 'id' => $save_id, 'view' => $this->input->post('view'), 'message' => lang('record_saved'), "add_type" => $add_type));
	...
	function view($form_id = 0)
	{
		...
		if ($form_id) {
			...
			if ($form_info) {
				...
				$view_data["categorias_de_formulario"] = $this->Categories_model->get_categories_of_material_of_form($form_id)->result();
				// USUARIOS
				$usuarios_formulario = json_decode($view_data['form_info']->usuarios, true);
				$array_usuarios_formulario = array();
				foreach ($usuarios_formulario as $id_usuario_formulario) {
					$usuario_formulario = $this->Users_model->get_one($id_usuario_formulario);
					$array_usuarios_formulario[] = $usuario_formulario->first_name . " " . $usuario_formulario->last_name;
				}
				$view_data["usuarios_formulario"] = $array_usuarios_formulario;
	...
	function view_data()
	{
		...
		if ($id_record) {
			...
			if (!$form_info->fijo) {
				...
				$columns = $this->Fixed_fields_model->get_all_where(
					array(
	...
	function view_list_data($id_record = 0)
	{
		...
		if ($form_info->fijo == 0) {
			...
			$columnas = $this->Fixed_fields_model->get_all_where(
				array(
	...
	function get_fields_form_view($field_type, $field_info)
	{
		...
		if ($field_type->nombre == "Selección desde Mantenedora") {
			...
			if ($id_mantenedora == "waste_transport_companies") {
				$datos = $this->Fixed_feeder_waste_transport_companies_values_model->get_details(
					array(
						"id_client" => $id_cliente,
						"id_project" => $id_proyecto
					)
				...
				$datos = $this->Fixed_feeder_waste_receiving_companies_values_model->get_details(
					array(
						"id_client" => $id_cliente,
						"id_project" => $id_proyecto
					)
	...
	function get_users_of_project()
	{

		$id_project = $this->input->post("id_project");
		$usuarios_proyecto = $this->Users_model->get_users_of_project($id_project)->result_array();
		$array_users = array();
		$array_users_selected = array();
		foreach ($usuarios_proyecto as $usuario) {
			$array_users[$usuario["id"]] = $usuario["first_name"] . " " . $usuario["last_name"];
			$array_users_selected[] = $usuario["id"];
		}
		...
		$html .= '<div class="col-md-9">';
		$html .= form_multiselect("users[]", $array_users, $array_users_selected, "id='users' class='multiple' multiple='multiple' data-rule-required='true', data-msg-required='" . lang('field_required') . "'");
		$html .= '</div>';
		$html .= '</div>';
	...
	/**
	 * modal_form_notify_review
	 * 
	 * Utilizado en las vistas de usuario de tipo "cliente" en los módulos de Registros Ambientales, Mantenedoras y Otros registros.
	 * Abre un modal que permite seleccionar un usuario y notificarle que los elementos del formulario (el que se esté notificando de uno de los 3 tipos) 
	 * están listos para ser revisados. La notificación será por correo electrónico, y el usuario a notificar debe ser miembro
	 * del proyecto asociado al formulario que se está notificando.
	 * 
	 * @author Gustavo Pinochet Altamirano
	 * 
	 */
	function modal_form_notify_review()
	{
		...
		$view_data["id_form"] = $id_form;
		$form = $this->Forms_model->get_details(array("id" => $id_form))->row();

		// MIEMBROS DE FORMULARIO
		$array_form_members = array("" => "-");
		$form_members = json_decode($form->usuarios);
		foreach ($form_members as $id_user) {
			$user = $this->Users_model->get_one($id_user);
			$array_form_members[$user->id] = $user->first_name . " " . $user->last_name;
		}
		$view_data["array_form_members"] = $array_form_members;

		$this->load->view('forms/notify_review/modal_form', $view_data);

	}

	/**
	 * send_notify_review_email
	 * 
	 * @author Gustavo Pinochet Altamirano
	 * @access public
	 * @return boolean true si el correo se ha enviado exitosamente.
	 */
	function send_notify_review_email_old()
	{
		...
		$id_reporting_user = $this->login_user->id;
		$reporting_user = $this->Users_model->get_one($id_reporting_user);
		$reporting_user_name = $reporting_user->first_name . " " . $reporting_user->last_name;

		$id_form = $this->input->post("id_form");
		$form = $this->Forms_model->get_details(array("id" => $id_form))->row();

		$email_template = $this->Email_templates_model->get_final_template("form_notify_review");
		$subject = $email_template->subject;

		$parser_data["SITE_URL"] = get_uri();
		$parser_data_signature["SITE_URL"] = get_uri();
		$signature_message = $this->parser->parse_string($email_template->signature, $parser_data_signature, TRUE);
		$parser_data["SIGNATURE"] = $signature_message;
		$parser_data["USER_TO_NOTIFY_NAME"] = $user_name;
		$parser_data["REPORTING_USER_NAME"] = $reporting_user_name;
		$parser_data["FORM_NAME"] = $form->nombre;

		if ($form->id_tipo_formulario == 1) { // REGISTRO AMBIENTAL
			$parser_data["FORM_URL"] = get_uri("environmental_records/view/" . $form->id);
		} else if ($form->id_tipo_formulario == 2) { // MANTENEDORA
			$parser_data["FORM_URL"] = get_uri("feeders/view/" . $form->id);
		} else if ($form->id_tipo_formulario == 3) { // OTRO REGISSTRO
			$parser_data["FORM_URL"] = get_uri("other_records/view/" . $form->id);
		}

		$parser_data["FORM_TYPE"] = $form->tipo_formulario;
		$parser_data["CONTACT_URL"] = get_uri("contact");

		$message = $this->parser->parse_string($email_template->message, $parser_data, TRUE);

		$send_app_mail = send_app_mail($user_email, $subject, $message);

		if ($send_app_mail) {

			$id_project = $form->id_proyecto;
			$id_client = $this->Projects_model->get_one($id_project)->client_id;
			$id_sender_user = $this->login_user->id;
			...
			$data_log = array(
				...
				"id_receiving_user" => $id_user,
				"created_at" => get_current_utc_time(),
			);
			$save_log = $this->Form_review_notifications_log_model->save($data_log);

			echo json_encode(array('success' => true, 'message' => lang("notification_sent")));
		} else {
			echo json_encode(array('success' => false, 'message' => lang('error_occurred')));
		}
	...
	function send_notify_review_email()
	{

		$id_form = $this->input->post("id_form");
		$form = $this->Forms_model->get_details(array("id" => $id_form))->row();
		$users_array = json_decode($form->usuarios);
		// $users_array = $this->Users_model->get_users_of_projects($form->id_proyecto); 

		foreach ($users_array as $user_value) {
			$id_user = $user_value;

			$user = $this->Users_model->get_one($id_user);

			if ($id_user == 9 || $user->id_profile == 4) {
