...
class Footprints extends MY_Controller {
    ...
    function index() {
		...
		// FILTRO TIPO UNIDAD
		$array_tipos_unidad[] = array("id" => "", "text" => "- ".lang("unit_type")." -");
		$tipos_unidad = $this->Unity_type_model->get_dropdown_list(array("nombre"), "id");
		foreach($tipos_unidad as $id => $nombre){
			$array_tipos_unidad[] = array("id" => $id, "text" => $nombre);
		}
		$view_data['tipos_unidad_dropdown'] = json_encode($array_tipos_unidad);
		
		// FILTRO UNIDAD
		$array_unidades[] = array("id" => "", "text" => "- ".lang("unit")." -");
		$unidades = $this->Unity_model->get_dropdown_list(array("nombre"), "id");
		foreach($unidades as $id => $nombre){
			$array_unidades[] = array("id" => $id, "text" => $nombre);
		}
		$view_data['unidades_dropdown'] = json_encode($array_unidades);

        $this->template->rander("footprints/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Footprints_model->get_one($id);
		
		$view_data['tipos_unidad'] = array("" => "-") + $this->Unity_type_model->get_dropdown_list(array("nombre"), "id");
		...
		$view_data["iconos"] = directory_map('./assets/images/impact-category/');
		
        $this->load->view('footprints/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if($id){
            $save_id = $this->Footprints_model->save($data, $id);
        }else{
            $save_id = $this->Footprints_model->save($data);  
        }
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Footprints_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Footprints_model->delete($id)) {
    ...
    function list_data() {
        ...
        $list_data = $this->Footprints_model->get_details($options)->result();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Footprints_model->get_details($options)->row();
    ...
    private function _make_row($data) {
		...
		$tipo_unidad = $this->Unity_type_model->get_one($data->id_tipo_unidad);
		$unidad = $this->Unity_model->get_one($data->id_unidad);
		
		/* <img heigth='20' width='20' src='/assets/images/impact-category/<?php echo $model_info->icono; ?>'/> &nbsp;<?php echo $model_info->icono; ?> */
		
        $row_data = array($data->id,
            modal_anchor(get_uri("footprints/view/" . $data->id), $data->nombre, array("title" => lang('view_footprint'))), $tipo_unidad->nombre, $unidad->nombre,
			$data->indicador, "<img heigth='20' width='20' src='/assets/images/impact-category/$data->icono' />"."&nbsp;".$data->icono
        );

        $row_data[] = modal_anchor(get_uri("footprints/view/" . $data->id), "<i class='fa fa-eye'></i>", array("title" => lang('view_footprint'))) .
        modal_anchor(get_uri("footprints/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_footprint'), "data-post-id" => $data->id))

                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_footprint'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("footprints/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($id_footprint = 0) {
       

        if ($id_footprint) {
            $options = array("id" => $id_footprint);
            $model_info = $this->Footprints_model->get_details($options)->row();
			
            if ($model_info) {
                $view_data['model_info'] = $model_info;
				$view_data['tipo_unidad'] = $this->Unity_type_model->get_one($view_data['model_info']->id_tipo_unidad);
				$view_data['unidad'] = $this->Unity_model->get_one($view_data['model_info']->id_unidad);
				
                $this->load->view('footprints/view', $view_data);
            } else {
                show_404();
            }
