 ...
 * @return number
 */
if (!function_exists('to_number_project_format')) {

    function to_number_project_format($number = 0, $project_id) {
		$thousands_separator = get_setting_mimasoft($project_id, "thousands_separator");
        $decimal_separator = get_setting_mimasoft($project_id, "decimals_separator");
		$decimal_number = get_setting_mimasoft($project_id, "decimal_numbers");
		$splitString = explode('.', $number);
		if(count($splitString) > 1){
			$num_decimales = strlen($splitString[count($splitString)-1]);
		}
		
		if($thousands_separator == 1){
			return number_format($number, $decimal_number, ",", ".");
		} else {
            return number_format($number, $decimal_number, ".", ",");
        }
    }
...
if (!function_exists('to_number_client_format')) {

    function to_number_client_format($number = 0, $client_id, $decimal_numbers = true) {
		$thousands_separator = get_setting_client_mimasoft($client_id, "thousands_separator");
        $decimal_separator = get_setting_client_mimasoft($client_id, "decimals_separator");
		$decimal_number = ($decimal_numbers) ? get_setting_client_mimasoft($client_id, "decimal_numbers") : 0;
		$splitString = explode('.', $number);
		if(count($splitString) > 1){
			$num_decimales = strlen($splitString[count($splitString)-1]);
		}
