...
if (!function_exists('upload_file_to_temp')) {
    ...
    function upload_file_to_temp($name = 'file', $options = array()) {
        ...
        if (!empty($_FILES)) {
            ...
            $waste_manifest_file = get_array_value($options, "waste_manifest_file");
			...
			} elseif($waste_manifest_file) {
				$file_name = "waste_manifest_file-".$_FILES[$name]['name'];
			} else {
				$file_name = $_FILES[$name]['name'];
            }
...
if (!function_exists('download_app_files')) {
    ...
    function download_app_files($directory_path, $serialized_file_data, $remove_prefix = true) {
        $ci = get_instance();
