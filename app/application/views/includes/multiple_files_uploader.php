<div class="form-group">
    <div class="col-sm-12 col-md-12 p0">
        <div id="file-upload-dropzone_<?php echo $id_campo;?>" class="dropzone mb15">

        </div>
        <div id="file-upload-dropzone-scrollbar_<?php echo $id_campo;?>">
            <div id="uploaded-file-previews_<?php echo $id_campo;?>">
                <div id="file-upload-row_<?php echo $id_campo;?>" class="box">
                    ...
                    <div class="box-content">
                        ...
                        <input class="file-count-field" type="hidden" name="<?php echo $html_name; ?>[]" value="" <?php echo $obligatorio; ?> />
                    </div>
                </div>
            </div>
        </div>
        <!--
        <input class="file_validation" type="hidden" name="<?php echo $html_name; ?>_validation" value="" <?php echo $obligatorio; ?>/>
    	-->
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var previewNode = document.querySelector("#file-upload-row_<?php echo $id_campo;?>");
        previewNode.id = "";
        var previewTemplate = previewNode.parentNode.innerHTML;
        previewNode.parentNode.removeChild(previewNode);

        var dropzoneId="#file-upload-dropzone_<?php echo $id_campo;?>";

        var projectFilesDropzone = new Dropzone(dropzoneId, {
            url: "<?php echo $upload_url; ?>",
			params: {cid: "<?php echo $id_campo;?>"},
            thumbnailWidth: 80,
            thumbnailHeight: 80,
            parallelUploads: 20,
            //maxFilesize: 3000,
            ...
            dictDefaultMessage: '<?php echo lang("single_file_upload_instruction"); ?>' + '<br><br>' +'<?php echo lang("maximum_each_file_size").": ".get_setting("max_file_size")."MB"; ?>',
            autoQueue: true,
            previewsContainer: "#uploaded-file-previews_<?php echo $id_campo;?>",
            clickable: true,
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
							...
							/*
							$("input[name='<?php echo $html_name; ?>_validation']").val(file.name);
                            done();
            ...
            fallback: function () {
                ...
                $("#file-upload-dropzone_<?php echo $id_campo;?>").hide();
                ...
                $(dropzoneId).closest('form').find(".modal-footer").on("click", "#add-more-file-button", function () {
                    var newFileRow = "<div class='file-row pb10 pt10 b-b mb10'>"
                            ...
                            + "</div>";
                    $("#uploaded-file-previews_<?php echo $id_campo;?>").prepend(newFileRow);
                });
                $("#add-more-file-button").trigger("click");
                $("#uploaded-file-previews_<?php echo $id_campo;?>").on("click", ".remove-file", function () {
