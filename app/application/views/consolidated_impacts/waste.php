...
<?php 
	...
	// RESIDUO VOLUMEN
	foreach($array_id_materiales_valores_volumen_residuo as $id_proyecto => $datos_proyectos_volumen){
				...
				foreach($datos_categorias_volumen as $id_categoria => $arreglo_valores){
					...
					foreach($years as $year){
						...
						if($subproyecto->id_proyecto == $id_proyecto){
							$array_material_subprojects_values_year_volumen_residuo[$nombre_material][$nombre_categoria][$array_projects[$id_proyecto]][$array_subprojects[$id_subproyecto]][$year] += $valor;
						}
					}
				}
			}
	...
	// RESIDUO MASA
	foreach($array_id_materiales_valores_masa_residuo as $id_proyecto => $datos_proyectos_masa){
			...
			foreach($datos_materiales_masa as $id_material => $datos_categorias_masa){
				...
				foreach($datos_categorias_masa as $id_categoria => $arreglo_valores){
					...
					foreach($years as $year){
						...
						if($subproyecto->id_proyecto == $id_proyecto){
							$array_material_subprojects_values_year_masa_residuo[$nombre_material][$nombre_categoria][$array_projects[$id_proyecto]][$array_subprojects[$id_subproyecto]][$year] += $valor;
						}
					}
				}
			}
							...
							<div id="tabla_residuo_volumen_subprojects" class="col-md-12 p0">
								<div class="page-title p10" style="border-bottom: none !important;">
									...
									<table id="table-residuo_volumen_subproject" class="table table-responsive table-striped">
										...
										<tbody>
											<?php foreach($array_material_subprojects_values_year_volumen_residuo as $nombre_material => $array_data_categories) { ?>
							...
							<div id="tabla_residuo_masa_subprojects" class="col-md-12 p0">
								<div class="page-title p10" style="border-bottom: none !important;">
									...
									<table id="table-residuo_masa_subproject" class="table table-responsive table-striped">
										...
										<tbody>
											<?php foreach($array_material_subprojects_values_year_masa_residuo as $nombre_material => $array_data_categories) { ?>
												<?php foreach($array_data_categories as $name_category => $array_data_projects) { ?>
													<?php foreach($array_data_projects as $name_project => $array_data_subprojects) { ?>
														<?php foreach($array_data_subprojects as $name_subproject => $array_data) { ?>
															<tr>
