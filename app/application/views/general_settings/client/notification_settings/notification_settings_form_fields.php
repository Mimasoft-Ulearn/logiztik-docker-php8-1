...
<input type="hidden" name="id_notif_config_audit" value="<?php echo $id_notif_config_audit; ?>" />
<input type="hidden" name="id_notif_config_close" value="<?php echo $id_notif_config_close; ?>" />
<input type="hidden" name="id_notif_config_send_email" value="<?php echo $id_notif_config_send_email; ?>" />
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#notification_config_client-form .select2').select2();
		...
		<?php foreach($eventos as $evento => $opciones) { ?>
			...
			$("#groups_<?php echo $evento; ?>").change(function(){
				...
				$.ajax({
					url:  '<?php echo_uri("general_settings/get_users_of_groups") ?>',
					type:  'post',
					data: {id_client: id_client, groups: groups, evento: evento},
					//dataType:'json',
