...
<input type="hidden" name="view" value="<?php echo isset($view) ? $view : ""; ?>" />
    ...
    <div class="form-group">
		...
		<?php
			...
			$arraySelected2["fijo_1"] = lang("permitting_number");
			...
			$arraySelected2["fijo_4"] = lang("entity");
			$array_campos_ocupados[] = "fijo_4";

            foreach($campos_permiso as $innerArray){
		...
		<?php if ($id_permiso) { ?>
			...
			/* $('#fields').change(function(){
				...
				if(selected){
					...
					$('#fields').removeAttr('data-msg-required');
					alert("hay seleccionados :D");
				} else {
					$('#fields').attr('data-rule-required', 'true');
					$('#fields').attr('aria-required', 'true');
					$('#fields').attr('data-msg-required');
			...
			var primer_evaluado = '<?php echo $evaluados_permiso[0]["nombre_evaluado"] ?>';
			$('#evaluado').val(primer_evaluado);
			var evaluados_permiso = [];
			
			<?php foreach($evaluados_permiso as $evaluado){ ?>
				
				var nombre_evaluado = '<?php echo $evaluado["nombre_evaluado"] ?>';	
				
				if(primer_evaluado != nombre_evaluado){	
					evaluados_permiso.push(nombre_evaluado);
