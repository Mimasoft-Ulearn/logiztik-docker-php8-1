<?php
defined('BASEPATH') OR exit('No direct script access allowed');
?>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>404</title>
<style type="text/css">

.bgc-white, .bgcH-white:hover {
    background-color: #fff!important;
}
.l-0 {
    left: 0;
}

.t-0 {
    top: 0;
}

.p-30 {
    padding: 30px!important;
}

.pos-a {
    position: absolute!important;
}


.jc-c {
    -webkit-box-pack: center;
    -ms-flex-pack: center;
    justify-content: center;
}

.ai-c {
    -webkit-box-align: center;
    -ms-flex-align: center;
    align-items: center;
}

.fxd-r, .fxd-rr {
    -webkit-box-orient: horizontal;
}

.fxd-r {
    -webkit-box-direction: normal;
    -ms-flex-direction: row;
    flex-direction: row;
}
.fxw-w {
    -ms-flex-wrap: wrap;
    flex-wrap: wrap;
}

.d-f, .layers {
    display: -webkit-box;
    display: -ms-flexbox;
    display: flex;
}

.w-100 {
    width: 100%!important;
}

.h-100 {
    height: 100%!important;
}

*, :after, :before {
    -webkit-box-sizing: border-box;
    box-sizing: border-box;
}
div {
    display: block;
}
.mR-60 {
    margin-right: 60px!important;
}


.fxd-c, .fxd-cr {
    -webkit-box-orient: vertical;
}

.fxd-c {
    -webkit-box-direction: normal;
    -ms-flex-direction: column;
    flex-direction: column;
}

element.style {
    font-size: 60px;
}


.c-red-500, .cH-red-500:hover {
    color: #00b393!important;
}

.lh-1 {
    line-height: 1!important;
}

.fw-900 {
    font-weight: 900!important;
}

.mB-30 {
    margin-bottom: 30px!important;
}

h1, h2, h3, h4, h5, h6 {
    letter-spacing: .5px;
}

.h1, h1 {
    font-size: 35px;
    font-size: 2.1875rem;
}

.h1, .h2, .h3, .h4, .h5, .h6, h1, h2, h3, h4, h5, h6 {
    margin-bottom: 8px;
    margin-bottom: .5rem;
    font-family: inherit;
    font-weight: 500;
    line-height: 1.2;
    color: inherit;
}

h1, h2, h3, h4, h5, h6 {
    margin-top: 0;
    margin-bottom: 8px;
    margin-bottom: .5rem;
}

h1 {
    display: block;
    font-size: 2em;
    -webkit-margin-after: 0.67em;
    -webkit-margin-start: 0px;
    -webkit-margin-end: 0px;
    font-weight: bold;
}
.c-grey-900, .cH-grey-900:hover {
    color: #313435!important;
}

.fsz-lg {
    font-size: 22.4px!important;
    font-size: 1.4rem!important;
}

.tt-c {
    text-transform: capitalize!important;
}

.mB-10 {
    margin-bottom: 10px!important;
}


.h3, h3 {
    font-size: 24.5px;
    font-size: 1.53125rem;
}
h3 {
    display: block;
    font-size: 1.17em;
    -webkit-margin-after: 1em;
    -webkit-margin-start: 0px;
    -webkit-margin-end: 0px;
    font-weight: bold;
}




.c-grey-700, .cH-grey-700:hover {
    color: #72777a!important;
}

.fsz-def {
    font-size: 16px!important;
    font-size: 1rem!important;
}


p {
    font-family: Roboto,-apple-system,system-ui,BlinkMacSystemFont,Segoe UI,Oxygen,Ubuntu,Cantarell,Fira Sans,Droid Sans,Helvetica Neue,Arial,sans-serif;
    line-height: 1.9;
}

p {
    margin-top: 0;
    margin-bottom: 16px;
    margin-bottom: 1rem;
}

user agent stylesheet
p {
    display: block;
    -webkit-margin-before: 1em;
    -webkit-margin-after: 1em;
    -webkit-margin-start: 0px;
    -webkit-margin-end: 0px;
}
.btn-primary {
    color: #fff;
    background-color: #00b393;
    border-color: #2196f3;
}
.btn {
    display: inline-block;
    font-weight: 400;
    text-align: center;
    white-space: nowrap;
    vertical-align: middle;
    -webkit-user-select: none;
    -moz-user-select: none;
    -ms-user-select: none;
    user-select: none;
    border: 1px solid transparent;
    padding: 6px 12px;
    padding: .375rem .75rem;
    font-size: 14px;
    font-size: .875rem;
    line-height: 1.5;
    border-radius: .25rem;
    -webkit-transition: background-color .15s ease-in-out,border-color .15s ease-in-out,-webkit-box-shadow .15s ease-in-out;
    transition: background-color .15s ease-in-out,border-color .15s ease-in-out,-webkit-box-shadow .15s ease-in-out;
    -o-transition: background-color .15s ease-in-out,border-color .15s ease-in-out,box-shadow .15s ease-in-out;
    transition: background-color .15s ease-in-out,border-color .15s ease-in-out,box-shadow .15s ease-in-out;
    transition: background-color .15s ease-in-out,border-color .15s ease-in-out,box-shadow .15s ease-in-out,-webkit-box-shadow .15s ease-in-out;
}
body, html, html a {
    -webkit-font-smoothing: antialiased;
}

a {
	color: #0f9aee;
    -webkit-transition: all .3s ease-in-out;
    -o-transition: all .3s ease-in-out;
    transition: all .3s ease-in-out;
}

[role=button], a, area, button, input:not([type=range]), label, select, summary, textarea {
    -ms-touch-action: manipulation;
    touch-action: manipulation;
}

a {
    color: #2196f3;
    text-decoration: none;
    background-color: transparent;
    -webkit-text-decoration-skip: objects;
}

user agent stylesheet
a:-webkit-any-link {
    color: -webkit-link;
    cursor: pointer;
    text-decoration: underline;
}
body, h1, h2, h3, h4, h5, h6 {
    font-family: Roboto,-apple-system,system-ui,BlinkMacSystemFont,Segoe UI,Oxygen,Ubuntu,Cantarell,Fira Sans,Droid Sans,Helvetica Neue,Arial,sans-serif;
    line-height: 1.5;
}

body {
	margin: 0;
	display: block;
	font-weight: 400;
	text-align: left;
    font-size: 14px;
    color: #72777a;
    letter-spacing: .2px;
    overflow-x: hidden;
	background-color: #fff;
}

*, :after, :before {
    -webkit-box-sizing: border-box;
    box-sizing: border-box;
}

</style>
  </head>
  <body class="app">
    <div class='pos-a t-0 l-0 bgc-white w-100 h-100 d-f fxd-r fxw-w ai-c jc-c pos-r p-30'>
      <div class='mR-60'>
        <img src='<?php echo get_file_uri("assets/images/404.png");?>'/>
      </div>

      <div class='d-f jc-c fxd-c'>
        <h1 class='mB-30 fw-900 lh-1 c-red-500' style="font-size: 60px;">404</h1>
        <h3 class='mB-10 fsz-lg c-grey-900 tt-c'><?php echo lang("error_404_title");?></h3>
        <p class='mB-30 fsz-def c-grey-700'><?php echo lang("error_404_message");?></p>
        <div>
          <a href="<?php echo get_uri("inicio_projects")  /*get_uri("inicio_projects")*/?>" type='primary' class='btn btn-primary'><?php echo lang("error_404_button"); ?></a>
        </div>
      </div>
    </div>
  </body>
</html>
